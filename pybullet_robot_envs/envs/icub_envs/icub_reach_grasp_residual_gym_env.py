...
class iCubReachGraspResidualGymEnv(gym.Env):
    ...
    def apply_action(self, action):
        ...
        if self.last_approach_step and self._grasping_step > 0:
            ...
            f_big = [f for f in ct_forces[:-1] if f >= 5.]
            f_big_thumb = ct_forces[-1] >= 5.

            if len(f_big) >= 2 and f_big_thumb:
                self._grasping_step = 0
    ...
    def _termination(self, w_obs):
        ...
        # rew 2: contact on at least two finger tips + thumb tip
        fingers_in_contact, curr_forces = self._robot.check_contact_fingertips(self._world.obj_id)
        is_grasping = fingers_in_contact >= 3 and curr_forces[-1] > 0
