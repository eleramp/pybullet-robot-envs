...
class Camera:
    ...
    def __init__(self):
        ...
        self.projection_matrix = (2.0 * self.fx / self.width, 0.0,  0.0, 0.0,
                                  ...
                                  1.0 - 2.0 * self.cx / self.width, 2.0 * self.cy / self.height - 1.0, (self.far + self.near) / (self.near - self.far), -1.0,
    ...
    def calc_point_cloud(self, id, client_id, step_x, step_y, plot_points=False, with_noise=False, obj_id=None):
        """
        ...
        :param step_x: number of pixels to be skipped in x direction
        :param step_y: number of pixels to be skipped in y direction
        :param plot_points: whether the points shall be plot in matplotlib (very slow)
        :param obj_id: the object_id to segment, if desired
        :return: points: np array (nb_pointsx3)
                 colors: np array (nb_pointsx4)
        """
        rgb_img, z, seg_mask = self.get_image(id, client_id, with_noise=with_noise)
        ...
        if obj_id is not None:
            pixels_obj_mask = seg_mask[:, :] == obj_id
            x = x[pixels_obj_mask]
            y = y[pixels_obj_mask]
            z = z[pixels_obj_mask]
            rgb_img = rgb_img[pixels_obj_mask]

        points = np.stack((x, y, z), axis=-1).reshape((x.shape[0], 3))
        colors = rgb_img.reshape((rgb_img.shape[0], 4))
    ...
    def get_image(self, robot_id, client_id, with_noise=False):
        """
